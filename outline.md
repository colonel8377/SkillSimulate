...
- **可证伪的 "so what" 预测**：distilled-skill agents 相对杠杆-1 强基线（含 Rich-Narrative persona）的增益，应集中在 Micro Behavior 层（Action Matrix Similarity / Caricature Index）与 emergent group dynamics（Exp 2 极化/冲突恢复）层
...
2. **实证贡献 (Exp 1)**：条件网格系统对比，**关键对比为 CADP distilled-skill（双 distiller: colleague + nuwa）相对杠杆-1 强基线（含 Rich-Narrative persona，复现 Scaling-Law arXiv:2510.11734）在 Micro Behavior 层的增益**（含 CADP minus Anti-patterns 机制消融 + Shuffled 置换检验 + distiller-robustness 对比 + Caricature 指标；8 主条件 + 附录消融）
...
**Gap**：杠杆 1 已被充分探索甚至饱和（Scaling-Law 主张其已足够），杠杆 2（inference-time intervention）在 social simulation 域空缺。CADP 占据杠杆 2，并在 Micro Behavior 层验证"杠杆 1 不够、需 inference-time intervention"。**这要求 CADP 必须在实验中正面击败 Scaling-Law 的 Rich-Narrative 强基线（§5.2 条件 4），否则论点失败**（§5.1 framing pilot 的 kill condition）。**注意**：此 kill condition 检验的是 distilled-skill + filter-retry 是否优于 lever-1 ceiling，不主张 filter-retry 是 weights-level 解决方案
...
- §5.2 Condition 4（Pop-Aligned 复现）vs Condition 7（CADP Full）：预期 CADP 在 Macro Topology（分布级）接近 Pop-Aligned，但在 Micro Behavior 层显著领先
...
- **触发器表示**：三类混合匹配
  ...
  - 类别 C（行为级）：分类器（轻量 logistic regression over 行为特征向量：stance shift rate, conflict engagement ratio 等）。**v1 状态（2026-07-09）**：infra 完整（`src/enforcement/behavioral_trigger_classifier.py` + `tier3_block.py::_check_behavioral` 双路径），但训练数据（`data/trigger_calibration/{dataset}.jsonl`，500 条 × 3 标注者）未生产，classifier 未挂载。主实验运行时 Cat C 自动降级为 legacy `->` substring fallback，而 YAML 中 `trigger_action_patterns` 全空 → **Cat C 实质不触发**。v1 Tier 3 仅 Cat A + Cat B active。Cat C 全流程（标注 + train + attach）deferred to v2
- **匹配逻辑**：任一 active 类别触发即标记为 violation；触发阈值通过 held-out 交互数据上的 F1 优化校准。v1 active = {A, B}
- **Trigger Calibration Protocol（独立校准实验 — §5 之外）**：
  ...
  - 报告指标：Precision / Recall / F1（v1 仅 trigger category A/B；**Cat C deferred to v2**，因 `data/trigger_calibration/` 尚未生产标注数据），而非仅 FP/FN rate
...
- **Framing pilot（review-driven, ARS 2026-06-19）**：在主实验之前运行 `configs/exp1_framing_pilot.yaml`——**4 条件 (descriptive / pop_aligned / rich_narrative / cadp_full) × 10 repeats × Wikipedia × 单模型**，仅测 Micro Behavior 层。**Pre-registered 决策规则**（unblinding 前冻结）：d ≥ 0.5 on Micro Behavior → method 主导 framing 可行；d ≈ 0.3 → benchmark 主导 framing；d < 0.3 → CADP 路线重构；CADP 输给 Pop-Aligned → benchmark framing + 重写 §2.3；**CADP 在 Micro Behavior 输给 Rich-Narrative（条件 4）→ 核心论点（inference-time intervention 必要性）失败，转 benchmark framing + 重写 §1.4/§2.2.5**。Framing pilot 与 §5.1 Power analysis 共享 effect-size 估计但作用不同
...
4. **Rich-Narrative Persona** (复现 Scaling-Law arXiv:2510.11734) — **杠杆-1 ceiling，CADP 核心论点存亡对照**：必须在 Micro Behavior 层显著超过 Rich-Narrative
...
- **报告（v1 scope）**：
  - **v1 active categories**: A 词法 + B 语义 的 Precision / Recall / F1
...
- **Distiller methodology 对比（条件 5 vs 6）**：CADP Full nuwa (5-layer) vs colleague (6-layer) 在 4 metric layer 的差异。若两者表现相近（Cohen's d < 0.3）→ CADP framework robust 到 distillation 结构；若 nuwa 显著更优 → 5-layer 结构对 social simulation 更适配（paper finding，§5.8 深入分析）
...
- **CADP vs Rich-Narrative（核心论点验证，§1.4/§2.2.5）**：在 Micro Behavior 层 CADP 是否显著超过 Scaling-Law 的"更丰富叙事"基线——这是 inference-time intervention 必要性的直接证据。预期：差距集中在 Micro Behavior，Macro Topology 可能接近
...
- 预测性保真度: ~~已删除 2026-07-13~~（regex-based event detection 不可靠；见 §5.3）
- Trigger calibration: **v1 仅 Cat A/B P/R/F1**；Cat C 行为级 + 跨域迁移 deferred to v2（§5.3.5）
...
> Table 1: 主结果表 (8 conditions × 4 metric layers × 1 model × 1 dataset)
...
- **杠杆 1 vs 杠杆 2 的边际增益分解（§1.4/§2.2.5 核心分析）**：沿杠杆 1 从 descriptive → segmentation → pop_aligned → rich_narrative 的 fidelity 增益曲线是否已饱和（plateau）？CADP（杠杆 2）相对 rich_narrative（杠杆 1 ceiling）的增益集中在哪一层？若杠杆 1 已 plateau 而 CADP 仍能提升 → 证明杠杆 2 是新方向；若杠杆 1 仍在上升 → CADP 须论证 enforcement 带来 description 无法触及的增益（如 Micro Behavior 层的 Action Matrix Similarity）
...
- Predictive fidelity 的 "so what" 论证 — **已删除 2026-07-13**：regex-based event detection 不可靠，Predictive Fidelity 层已移除。Micro Behavior 层的 Action Matrix Similarity + Caricature Index 承担 "so what" 论证角色
...
- **方法学循环性**：聚类→编译→验证基于同一社区数据。三重防御：(1) 置换检验（shuffle agent→skill assignment），(2) 跨数据集迁移 (Wikipedia→Reddit, 全组件 + 方法论两种迁移模式)，(3) 跨结构部分迁移（Reddit→GitHub，仅迁移可迁移组件 + 平台特定规则重编译；因目标侧重编译，非 zero-shot）。三者一致指向 CADP 优势时循环性影响可控。~~(4) held-out 事件预测~~ — 已删除 2026-07-13（regex-based event detection 不可靠）。
...
- **Category C 行为级 trigger deferred to v2（2026-07-09 新增）**：`src/enforcement/behavioral_trigger_classifier.py` infra 完整，`tier3_block.py::_check_behavioral` 双路径（spec classifier 优先 + legacy `->` substring fallback）。但 v1 未生产 `data/trigger_calibration/{dataset}.jsonl`（500 条 × 3 标注者），classifier 未挂载。主实验 Tier 3 仅 Cat A（关键词）+ Cat B（语义）active；YAML 中 `trigger_action_patterns` 全空 → legacy fallback 无模式可配 → **Cat C 实质不触发**。v1 报告的 Tier 3 P/R/F1 仅 A/B；Cat C 全流程（标注 + train + attach）deferred to v2。这不是 spec 违反（outline 从未声称 v1 已验证 Cat C），但 reviewer 可能问"三层 filter-retry 是否全部 active"——本文在 §4.4.1 / §5.3.5 / §5.7 明示"v1 active = {A, B}"。
...
| # | Content | Section |
|---|---------|---------|
| Table 1 | 13 conditions × 4 metric layers main results | 5.7 |
...
| Table 5 | Trigger calibration results (v1: Cat A/B P/R/F1 on Wikipedia; Cat C + 跨域迁移 deferred to v2) | 5.3.5 |
...
| Figure 3 | Radar chart — 13 conditions × 4 metric layers | 5.7 |
